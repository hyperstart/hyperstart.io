...
import { JsModules, JsModule } from "./modules"
import { ModuleSource, tryParse, getName } from "./utils"
...
import { HELPERS_ID, PREFIX } from "./helpers"
...
export function transformCommonjsModule(
  parse,
  code: string,
  id: string,
  isEntry: boolean,
  ignoreGlobal: boolean,
  ignoreRequire: (value: string) => boolean,
  sourceMap: boolean,
  ...
  const ast = jsModule.ast || tryParse(parse, code, id)
  const magicString = new MagicString(code)

  const required: StringMap<RequiredSource> = {}
  ...
  function isRequireStatement(node) {
    ...
    if (
      ...
      return // TODO handle these weird cases?
    if (ignoreRequire(node.arguments[0].value)) return

    return true
  }

  function getRequired(node, name?: string): RequiredSource {
    const source: string =
      node.arguments[0].type === "Literal"
  ...
  requiredExports.forEach(addExport)
